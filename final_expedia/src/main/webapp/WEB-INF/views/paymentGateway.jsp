...
$(document).ready(function() {
   ...
   IMP.init('imp20285384');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드". 
   // 결제요청하기
   IMP.request_pay({
       pg : 'html5_inicis', // 결제방식 PG사 구분
       pay_method : 'card',	// 결제 수단
       merchant_uid : 'merchant_' + new Date().getTime(), // 가맹점에서 생성/관리하는 고유 주문번호
       name : '신라호텔 디럭스 더블룸 결제',	 // 코인충전 또는 order 테이블에 들어갈 주문명 혹은 주문 번호. (선택항목)원활한 결제정보 확인을 위해 입력 권장(PG사 마다 차이가 있지만) 16자 이내로 작성하기를 권장
       amount : 100,	//100   // '${coinmoney}'  결제 금액 number 타입. 필수항목. 
       buyer_email : '${requestScope.paraMap.email}',  // 구매자 email
       buyer_name : '${requestScope.paraMap.name}',	  // 구매자 이름 
       buyer_tel : '${requestScope.paraMap.mobile}',    // 구매자 전화번호 (필수항목)
       buyer_addr : '',  
       buyer_postcode : '',
	   ...
	   */
	   ...
	   //  rsp.success 뜻은 카드결제가 성공되어서 돈이 빠져나갔음을 의미!
